class FriendsController < ApplicationController
	before_action :find_user, only: [:add_friend, :accept_friend, :cancel_friend, :show]
	...
	def show
    if current_user.friends_with?(@friend) || @friend == current_user
		  @posts = @friend.posts.order(updated_at: :desc)
    else
      @posts = Post.non_friend_posts(@friend)
    end
	end
